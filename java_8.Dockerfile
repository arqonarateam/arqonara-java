...
LABEL org.opencontainers.image.description="Java 8 (Azul Zulu) image for Pterodactyl"
...
RUN set -e; \
    URL=$(curl -fsSL "https://api.azul.com/zulu/download/community/v1.0/bundles/latest/?java_version=8&os=linux&arch=x86&hw_bitness=64&bundle_type=jdk&ext=tar.gz" | jq -r ".url"); \
