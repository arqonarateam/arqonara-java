...
LABEL org.opencontainers.image.description="Java 11 (Azul Zulu) image for Pterodactyl"
...
RUN set -e; \
    URL=$(curl -fsSL "https://api.azul.com/zulu/download/community/v1.0/bundles/latest/?java_version=11&os=linux&arch=x86&hw_bitness=64&bundle_type=jdk&ext=tar.gz" | jq -r ".url"); \
